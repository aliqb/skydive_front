import { DocumentItem, DocumnetStatus } from "../../../models/account.models";
...
const AdminUserDocumentItem: React.FC<AdminUserDocumentItemProp> = ({
  ...
  const checkDocument = useCallback((id: string, approve: boolean) => {
    sendRequest(
      {
    ...
    const fetchFile = async (address: string) => {
      try {
        ...
        if (isImage) {
          ...
          setFileContent(documnetImagePlacholder);
        }
      } catch (error) {
  ...
  return (
    <SDCard className="border w-52 flex flex-col items-center mb-6">
      <div className="border w-40 ">
        ...
        {/* {documentData && documentData.status !==DocumnetStatus.NOT_LOADED && documentData.fileId && documentData.createdAt && (
      ...
      {documentData?.status === DocumnetStatus.PENDING && (
        <div className="flex justify-center gap-1 mt-auto">
          <SDButton color="success" size="xs" className="px-0 py-0" disabled={isPending}
          onClick={()=>checkDocument(documentData.id || '',true)}>
            تأیید
          </SDButton>
          <SDButton size="xs" className="px-0 py-0" color="failure" disabled={isPending}
